...
export function getUsers(query: string): Promise<Array<MentionFeedItem>> {
    return new Promise(resolve => {
        axios.post("/api/global/find", { query, usersOnly: true }).then(res => {
            const newData = res.data.results.map((user: DBAuthType) => {
                return {
                    ...user,
                    id: `@${user.name}`,
                    link: `/u/${user._id}`,
...
export function getPosts(query: string): Promise<Array<MentionFeedItem>> {
    return new Promise(resolve => {
