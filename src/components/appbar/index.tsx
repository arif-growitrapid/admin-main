...
export const menuFunc = (user?: AuthType) => [
  ...
  user?.permissions?.course_edit &&
    user.permissions.service_edit && {
      name: "Services",
      path: "/services",
      description:
        "Add, view, manage and monitor services and enrollments. Also manage courses and their enrollments.",
      options: [
        (user?.permissions?.service_edit || true) && {
          name: "Services",
          path: "/services",
          description: "Manage services and enrollments",
        },
        (user?.permissions?.course_edit || true) && {
          name: "Courses",
          path: "/courses",
  ...
  user?.permissions?.view_reports && {
    ...
    path: "/monitors",
    description:
      "View, manage and examine monitors data and reports. Well designed charts and graphs to help you understand your data.",
    options: [
      {
        name: "Analytics",
        path: "/analytics",
  ...
  user?.permissions?.database_manage && {
    ...
    path: "/database",
    description:
      "Manage database and collections as well as assets. Perform CRUD operations on database and collections.",
    options: [
      {
        name: "Sanity Studio",
        path: "/sanity-studio",
      ...
      },
      {
        ...
        path: "/assets",
        description:
          "Manage, add, delete all Assets (Images, Gifs, Videos etc).",
      },
    ],
  },
...
export default function AppBar({ theme }: { theme: string }) {
  ...
  const themeBroadcast = useRef(
    new BroadcastChannel<string>(config.theme_key, {
      should_receive_own_messages: true,
    })
  );
  ...
  return (
    <header className={`w-full h-auto border-b-2`}>
      <div
        className={`w-full h-auto py-2 px-6 flex flex-row items-center gap-2`}
      >
        ...
        <div
          className={`menu w-auto h-full hidden lg:flex flex-row items-center gap-2`}
        >
          <NavigationMenuBar />
        </div>
        ...
        <div className={`right w-auto h-full flex flex-row items-center gap-2`}>
          ...
          <Popover>
            ...
            <PopoverContent className="w-80 h-[70vh] mr-20">
              <div className="h-full flex flex-col">
                <h2 className="text-lg font-semibold text-muted-foreground">
                  Notifications
                </h2>
                <div className="flex-1 h-full flex-grow">
          ...
          <Dialog>
            ...
            <DialogContent className="w-[calc(100%-2rem)] max-w-[800px] h-full max-h-[600px] rounded-lg">
              <DialogHeader>
                ...
                <DialogDescription>
                  <p className="text-xs text-muted-foreground">
                    Below are the keyboard shortcuts that can be used to
            ...
            <NavigationMenuItem key={index}>
              {item.options ? (
                <>
                  ...
                  <NavigationMenuContent>
                    <h2 className="px-4 pt-2 text-lg font-semibold text-muted-foreground">
                      {item.name}
                    </h2>
                ...
                if (item.options)
                  return (
                    <AccordionItem
                      ...
                      className="bg-muted hover:bg-[hsl(var(--muted)/60%)] transition-colors rounded-md"
                    >
                      ...
                      <AccordionContent className="pb-2">
                        <p className="px-4 text-xs text-muted-foreground line-clamp-2">
                          {item.description}
                        </p>
                        ...
                        <ul className="w-full gap-2 flex flex-col items-stretch px-2 pt-2">
                          {item.options.map((option, index) => {
                            ...
                            return (
                              <li key={index}>
                                <Link
                                  className="block select-none rounded-md p-2 leading-none no-underline outline-none transition-colors hover:bg-accent hover:text-accent-foreground focus:bg-accent focus:text-accent-foreground cursor-pointer"
                                  href={`${item.path}${option.path}`}
                                  passHref
                                >
            ...
            <p className="flex justify-between py-2">
              ...
              <span className="text-xs font-medium text-muted-foreground ml-1">
                ...
                <a
                  ...
                  target="_blank"
                  className="hover:after:w-full after:w-0 after:h-[1px] after:bg-muted after:transition-all after:absolute after:-bottom-[2px] after:left-0 relative"
                >
                  Arif Sardar
                </a>
              </span>
