...
export async function createBlog(
  path?: string
): Promise<ServerFunctionResponse<boolean | null>> {
  try {
    ...
    const createdBlog = await collection.findOne({
      _id: createdBlogRef.insertedId,
    });
...
export async function getBlogsByUser(
  id: string
): Promise<ServerFunctionResponse<WithId<DBBlogPostType>[] | null>> {
  try {
    // Match permissions to view user
    // If the user has the permission to view user, then return the user
    ...
    if (!t)
      return Response(
        ...
        null,
        401,
        "You don't have the permission to view users"
      );
    const { session, isMatched, matches } = t;
    ...
    const should_view_draft =
      matches.includes("blogs_view_draft") || session.user.id === id;
  ...
  try {
    ...
    if (!t)
      return Response(
        ...
        null,
        401,
        "You don't have the permission to view blogs"
      );
    const { session, isMatched, matches } = t;
    ...
    if (
      ...
      return Response(
        ...
        null,
        401,
        "You don't have the permission to view this blog"
      );
...
export async function getBlogBySlug(
  ...
  try {
    ...
    const { session, isMatched, matches } = t ?? {
      session: null,
      isMatched: false,
      matches: ["blogs_view_published"] as [
        "blogs_view_published" | "blogs_view_draft",
      ],
    };
  ...
  try {
    ...
    if (!t)
      return Response(
        ...
        null,
        401,
        "You don't have the permission to like blogs"
      );
    const { session, isMatched, matches } = t;
...
export async function removeLikeBlog(
  blog_id: string,
  path?: string
): Promise<ServerFunctionResponse<boolean | null>> {
  try {
  ...
  try {
    ...
    if (!t)
      return Response(
        ...
        null,
        401,
        "You don't have the permission to save blogs"
      );
    const { session, isMatched, matches } = t;
    ...
    const is_saved_in_blog = blog.saved_by.find(
      (user) => user.id === session.user.id
    );
    const is_saved_in_user = user?.savedContent?.find(
      (blog) => blog.ref === blog_id
    );
  ...
  try {
    ...
    if (!t)
      return Response(
        ...
        null,
        401,
        "You don't have the permission to save/unsave blogs"
      );
    const { session, isMatched, matches } = t;
...
export async function viewBlog(
  blog_id: string,
  ip: string,
  path?: string
): Promise<ServerFunctionResponse<WithId<DBBlogPostType> | null>> {
  ...
  try {
    ...
    if (!t)
      return Response(
        ...
        null,
        401,
        "You don't have the permission to delete blogs"
      );
    const { session, isMatched, matches } = t;
    ...
    if (
      blog.author.id !== session.user.id &&
      !matches.includes("blogs_delete_others")
    )
      return Response(
        "error",
        null,
        401,
        "You don't have the permission to delete this blog"
      );
...
export async function changeVisibilityBlog(
  blog_id: string,
  path?: string
): Promise<ServerFunctionResponse<boolean | null>> {
  try {
    ...
    if (!t)
      return Response(
        ...
        null,
        401,
        "You don't have the permission to change the visibility of blogs"
      );
    const { session, isMatched, matches } = t;
    ...
    if (
      blog.author.id !== session.user.id &&
      !matches.includes("blogs_edit_others")
    )
      return Response(
        "error",
        null,
        401,
        "You don't have the permission to change the visibility of this blog"
      );
    ...
    return Response(
      ...
      true,
      200,
      "Blog visibility changed successfully"
    );
  } catch (error) {
    console.error(error);
    return Response("error", null, 500, "Internal server error");
  ...
  try {
    ...
    if (!t)
      return Response(
        ...
        null,
        401,
        "You don't have the permission to update blogs"
      );
    const { session, isMatched, matches } = t;
    ...
    if (
      ...
      return Response(
        ...
        null,
        401,
        "You don't have the permission to update this blog"
      );
    ...
    const updatedBlog = await collection.findOneAndUpdate(
      ...
      },
      {
        $set: {
          ...
          is_seo_compatabile:
            data.is_seo_compatabile ?? blog.is_seo_compatabile,
        },
      },
      { returnDocument: "after" }
    );
